  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        ...
        // Get source chain ID
        const promises = data.conditions.map(
          (condition: any) =>
            new Promise((resolve, reject) => {
              axios
                ...
                .then((response2) => {
                  condition.sourceChainId = parseInt(
                    response2.data[0].chain.split("_")[1]
                  );
                  resolve(undefined);
                })
                .catch(reject);
  ...
  return (
    <div className="min-h-screen bg-red-900 text-white selection:bg-indigo-500 selection:text-white">
      ...
      {!proposal ? (
        ...
        <>
          <section
            className="gallery container relative z-10 mx-auto py-10 px-6"
            id="proposal"
          >
            <h3 className="mb-4 text-left text-3xl font-semibold underline decoration-amber-500/80 lg:text-left xl:text-4xl">
              Proposal Details
            </h3>
